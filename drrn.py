...
class DRRN_Agent:
    ...
    def update(self):
        ...
        if self.w_inv > 0:
            if self.type_inv == "decode":
                terms["Loss_id"], terms["Acc_id"] = self.network.inv_loss_decode(
                    batch.state, batch.next_state, nested_acts, hat=True
                )
            elif self.type_inv == "ce":
                terms["Loss_id"], terms["Acc_id"] = self.network.inv_loss_ce(
        ...
        if self.w_act > 0:
            terms["Loss_act"], terms["Acc_act"] = self.network.inv_loss_decode(
        ...
        if self.w_for > 0:
            if self.type_for == "l2":
                terms["Loss_fd"] = self.network.for_loss_l2(
                    batch.state, batch.next_state, nested_acts
                )
            elif self.type_for == "ce":
                terms["Loss_fd"], terms["Acc_fd"] = self.network.for_loss_ce(
                    batch.state, batch.next_state, nested_acts, batch.acts
                )
            elif self.type_for == "decode":
                terms["Loss_fd"], terms["Acc_fd"] = self.network.for_loss_decode(
                    batch.state, batch.next_state, nested_acts, hat=True
                )
    ...
    def load(self, path=None):
        ...
        try:
            ...
            state_dict = {
                k: v for k, v in state_dict.items() if any(part in k for part in parts)
            }
    ...
    def save(self, step=""):
        try:
            ...
            pickle.dump(
                self.memory, open(pjoin(self.save_path, step, "memory.pkl"), "wb")
            )
