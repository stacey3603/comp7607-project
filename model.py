...
class DRRN(torch.nn.Module):
    ...
    def for_predict(self, state_batch, acts):
        ...
        next_state_out = state_out + self.forward_dynamics(
            torch.cat((state_out, act_out), dim=1)
        )
    ...
    def inv_loss_ce(
        self, state_batch, next_state_batch, acts, valids, get_predict=False
    ):
        ...
        for i, j in enumerate(act_sizes):
            ...
            loss += F.cross_entropy(
                values.unsqueeze(0), torch.LongTensor([label]).to(device)
            )
        ...
        return (
            (loss / len(act_sizes), acc / len(act_sizes), predicts)
            if get_predict
            else (loss / len(act_sizes), acc / len(act_sizes))
        )

    def inv_loss_decode(
        self, state_batch, next_state_batch, acts, hat=True, reduction="mean"
    ):
        ...
        acts_tensor = (
            torch.from_numpy(acts_pad).type(torch.long).to(device).transpose(0, 1)
        )
        l, bs = acts_tensor.size()
        vocab = self.embedding.num_embeddings
        outputs = torch.zeros(l, bs, vocab).to(device)
        input, z = acts_tensor[0].unsqueeze(0), (
            act_out_hat if hat else act_out
        ).unsqueeze(0)
        for t in range(1, l):
        ...
        loss = F.cross_entropy(
            outputs.reshape(-1, vocab),
            acts_tensor.reshape(-1),
            ignore_index=0,
            reduction=reduction,
        )
        ...
        acc_token = (same & nonzero).float().sum() / (
            nonzero
        ).float().sum()  # token accuracy
        acc_action = (
            same.int().sum(0) == nonzero.int().sum(0)
        ).float().sum() / same.size(
            1
        )  # action accuracy
    ...
    def for_loss_ce(self, state_batch, next_state_batch, acts, valids):
        ...
        for i, j in enumerate(act_sizes):
            valid_out = valids_out[now : now + j]
            now += j
            next_states_out_hat = self.for_predict(
                [state_batch[i]] * j, [[_] for _ in valids[i]]
            )
    ...
    def for_loss_decode(self, state_batch, next_state_batch, acts, hat=True):
        ...
        vocab = self.embedding.num_embeddings
        outputs = torch.zeros(l, bs, vocab).to(device)
        input, z = next_state_tensor[0].unsqueeze(0), (
            next_state_out_hat if hat else next_state_out
        ).unsqueeze(0)
        for t in range(1, l):
            input = self.embedding(input)
            ...
            top = output.argmax(2)
            input = top
        outputs, next_state_tensor = outputs[1:].reshape(-1, vocab), next_state_tensor[
            1:
        ].reshape(-1)
    ...
    def act(self, states, act_ids, sample=True, eps=0.1):
        ...
        if sample:
            ...
            act_idxs = [
                ...
                else np.random.randint(len(vals))
