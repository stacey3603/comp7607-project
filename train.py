...
def evaluate_episode(agent, env):
    ...
    log(
        "Obs{}: {} Inv: {} Desc: {}".format(
            step, clean(ob), clean(info["inv"]), clean(info["look"])
        )
    )
    while not done:
...
def train(
    ...
    for step in range(1, max_steps + 1):
        action_ids, action_idxs, action_values = agent.act(
            states, valid_ids, sample=True, eps=0.05 ** (step / max_steps)
        )
        ...
        examples = [
            (action, value)
            for action, value in zip(infos[0]["valid"], action_values[0].tolist())
        ]
        examples = sorted(examples, key=lambda x: -x[1])
        log(
            "State  {}: {}".format(
                step, clean(obs[0] + infos[0]["inv"] + infos[0]["look"])
            )
        )
        ...
        if r_for > 0:
            reward_curiosity, _ = agent.network.inv_loss_decode(
                states,
                next_states,
...
def parse_args():
    ...
    parser.add_argument("--memory_alpha", default=0.4, type=float)
    ...
    parser.add_argument(
        "--perturb", default=0, type=int, help="perturb state and action"
    )
