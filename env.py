...
def modify(info):
    ...
    navis = (
        "north/south/west/east/northwest/southwest/northeast/southeast/up/down".split(
            "/"
        )
    )
...
class JerichoEnv:
    ...
    def __init__(
        self, rom_path, seed, step_limit=None, get_valid=True, cache=None, args=None
    ):
        ...
        self.random_rewards = (np.random.rand(10000) - 0.5) * 10.0
        self.objs = set()
    ...
    def paraphrase(self, s):
        if s in self.perturb_dict:
            return self.perturb_dict[s]
        with torch.no_grad():
